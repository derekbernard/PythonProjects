...
class Bot():
            ...
            while True:
                try:
                    ...
                    if not self.visited():
                        ...
                        self.writeLastVisited(self.driver.current_url)
                    else : 
                        print("already visited exiting")
                        sleep(900)
                        bot.login() 
                    break
                except:
    ...
    def visited(self):
        ...
        with open("lastVisited.txt") as fp:
            lines = fp.readlines()
            print("comparaing : " + url)
            print("with : " + lines[0])
            if url in lines[0]:
                fp.close()
                return True
    ...
    def executeAnalysis(self):
        ...
        if self.municipalAssessmentExist(text):
            if self.compareValueToAssessment(text) < 2:
                self.writeIdToFile(self.driver.current_url)
                self.sendEmail(self.driver.current_url)
    ...
    def writeLastVisited(self,url):
        f = open("lastVisited.txt", "a")
        f.write(url + "\n")
        f.close()
